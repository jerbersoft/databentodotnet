...
/// <summary>Reads and writes <see cref="Fraction"/> as its one-character wire code.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>A blank is a legal fraction on the wire, and this converter still rejects it.</b> The
/// <c>FRACCD</c> group lists a null-code entry and upstream models the field as
/// <c>Option&lt;Fraction&gt;</c>, so a blank means "no value" — which a non-nullable
/// <see cref="Fraction"/> field has no way to hold. A model field that may be blank is
/// <c>Fraction?</c> carrying <see cref="NullableFractionJsonConverter"/>; this one is for a field
/// that may not.
/// </para>
/// <para>
/// Attached to the type by <c>[JsonConverter]</c>, which is what the <see cref="System.Text.Json"/>
/// source generator reads — so <see cref="Fraction"/> is AOT-safe wherever a generated context
...
public sealed class FractionJsonConverter : JsonConverter<Fraction>
{
    ...
    /// with a message rather than silently becoming <c>default(Fraction)</c>.
    /// </summary>
    /// <remarks>
    ...
    {
        var code = ReferenceEnumCode.Read(ref reader, nameof(Fraction))
            ?? throw new JsonException(
                "Fraction has a blank value, but this field is not nullable. Declare the field as "
                + "Fraction? and give it NullableFractionJsonConverter, which reads a blank as null.");
