...
/// <summary>Reads and writes <see cref="ListingStatus"/> as its one-character wire code.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>A blank is rejected, and the dictionary is the reason.</b> The <c>LISTSTAT</c> group lists no
/// blank entry, and upstream declares the field <c>pub listing_status: ListingStatus</c>
/// (<c>security.rs:187</c>) rather than an <c>Option</c>.
/// </para>
/// <para>
/// Attached to the type by <c>[JsonConverter]</c>, which is what the <see cref="System.Text.Json"/>
/// source generator reads — so <see cref="ListingStatus"/> is AOT-safe wherever a generated context
...
public sealed class ListingStatusJsonConverter : JsonConverter<ListingStatus>
{
    ...
    /// with a message rather than silently becoming <c>default(ListingStatus)</c>.
    /// </summary>
    /// <remarks>
    ...
    {
        var code = ReferenceEnumCode.Read(ref reader, nameof(ListingStatus))
            ?? throw ReferenceEnumCode.Blank(nameof(ListingStatus), "LISTSTAT");

        return ReferenceWireStrings.TryParseListingStatus(code, out var value)
            ? value
            : throw ReferenceEnumCode.Unrecognised(nameof(ListingStatus), code);
    }
