...
/// <summary>Reads and writes <see cref="ListingSource"/> as its one-character wire code.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>A blank is rejected, and the dictionary is the reason.</b> The <c>LISTSOURCE</c> group lists
/// no blank entry, and upstream declares the field <c>pub listing_source: ListingSource</c>
/// (<c>security.rs:189</c>) rather than an <c>Option</c>.
/// </para>
/// <para>
/// Attached to the type by <c>[JsonConverter]</c>, which is what the <see cref="System.Text.Json"/>
/// source generator reads — so <see cref="ListingSource"/> is AOT-safe wherever a generated context
...
/// </para>
/// </remarks>
public sealed class ListingSourceJsonConverter : JsonConverter<ListingSource>
{
    /// <summary>
    /// <see langword="true"/>, so a JSON <c>null</c> reaches <see cref="Read"/> and is rejected
    /// with a message rather than silently becoming <c>default(ListingSource)</c>.
    /// </summary>
    /// <remarks>
    ...
    /// <inheritdoc/>
    public override ListingSource Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var code = ReferenceEnumCode.Read(ref reader, nameof(ListingSource))
            ?? throw ReferenceEnumCode.Blank(nameof(ListingSource), "LISTSOURCE");

        return ReferenceWireStrings.TryParseListingSource(code, out var value)
            ? value
            : throw ReferenceEnumCode.Unrecognised(nameof(ListingSource), code);
    }

    /// <inheritdoc/>
    public override void Write(Utf8JsonWriter writer, ListingSource value, JsonSerializerOptions options) =>
