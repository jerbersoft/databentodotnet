...
/// <summary>Reads and writes <see cref="GlobalStatus"/> as its one-character wire code.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>A blank is rejected, and the dictionary is the reason.</b> The <c>GLOBSTATUS</c> group lists
/// no blank entry, and upstream declares the field <c>pub global_status: GlobalStatus</c>
/// (<c>corporate.rs:232</c>) rather than an <c>Option</c>.
/// </para>
/// <para>
/// Attached to the type by <c>[JsonConverter]</c>, which is what the <see cref="System.Text.Json"/>
/// source generator reads — so <see cref="GlobalStatus"/> is AOT-safe wherever a generated context
...
/// </para>
/// </remarks>
public sealed class GlobalStatusJsonConverter : JsonConverter<GlobalStatus>
{
    /// <summary>
    /// <see langword="true"/>, so a JSON <c>null</c> reaches <see cref="Read"/> and is rejected
    /// with a message rather than silently becoming <c>default(GlobalStatus)</c>.
    /// </summary>
    /// <remarks>
    ...
    /// <inheritdoc/>
    public override GlobalStatus Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var code = ReferenceEnumCode.Read(ref reader, nameof(GlobalStatus))
            ?? throw ReferenceEnumCode.Blank(nameof(GlobalStatus), "GLOBSTATUS");

        return ReferenceWireStrings.TryParseGlobalStatus(code, out var value)
            ? value
            : throw ReferenceEnumCode.Unrecognised(nameof(GlobalStatus), code);
    }

    /// <inheritdoc/>
    public override void Write(Utf8JsonWriter writer, GlobalStatus value, JsonSerializerOptions options) =>
